...
## SI 206 - W17 - HW3
## COMMENT WITH:
## Your section day/time: 002/TH3-4PM
...
def parse_counted_words(s):
    ...
    regex = r"(\d+\s\D\S\w+)"
    countword = re.findall(regex, s)

    if countword != []:
        last = countword[-1].split()
...
reg = r"\w+\.\w+"
file_paths_num = len(re.findall(reg, data))
...
reg = r"^(\~|/)"
full_paths_num = len(re.findall(reg, data, re.MULTILINE))
...
reg = r"SI206/\w+\.py"
python_course_paths = len(re.findall(reg, data))
...
reg = r"\d\.(docx|xlsx)"
microsoft_files_num = len(re.findall(reg, data))
