...
table_spec += 'tweet_text TEXT, user_id TEXT, time_posted TIMESTAMP, retweets INTEGER)'
...
for each in umich_tweets:
	...
	tweet_id = each['id_str']
	user_id = each['user']['id_str']
	screen_name = each['user']['screen_name']
	num_favs = each['user']['favourites_count']
	description = each['user']['description']
	if len(each['entities']['user_mentions']) != 0:
		for a in each['entities']['user_mentions']:
			user_id = a['id_str']
			screen_name = a['screen_name']
			user_results = api.get_user(a['id'])
			num_favs = user_results['favourites_count']
			description = user_results['description']

	cur.execute('INSERT INTO Tweets (tweet_id, tweet_text, user_id, time_posted, retweets) VALUES (?, ?, ?, ?, ?)', (tweet_id, tweet_text, user_id, time_posted, retweets))
...
screen_names = []
for row in cur:
	[screen_names.append(a) for a in row]
