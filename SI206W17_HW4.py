...
f = open("nytimes_data.html", "r",encoding="utf-8")
nysting = (f.read().encode('utf-8'))
f.close()

soup = BeautifulSoup(nysting, "lxml")

headlines = []

for story_heading in soup.find_all(class_="story-heading"):
    if story_heading.a: 
        headlines.append(story_heading.a.text.replace("\n", " ").replace("\xe9", "'").strip().encode('utf-8').decode('UTF-8'))

nytimes_headlines = headlines[:9]
...
for title in soup.find_all("div", class_= "field-items"):
	if title.a:
		None
	else:
		a.append(title.text)
